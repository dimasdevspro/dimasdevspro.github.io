async function renderPortfolioHighlights() {
  ...
  try {
    const response = await fetch(
      "https://api.github.com/users/dimasdevspro/repos?per_page=100",
    );
    ...
    if (reposComHomepage.length === 0) {
      if (projectsMessage) {
        projectsMessage.textContent =
          "Nenhum projeto com homepage foi encontrado.";
      }
      return;
    }
    ...
    for (let modal of modals) {
      modal.addEventListener("click", function () {
        ...
        if (modalsContainer) {
          modalsContainer.classList.add("dimmed");
        }
        ...
        modalOverlay.querySelector(".modal_content h2").textContent =
          title.textContent;
    ...
    document
      ...
      .addEventListener("click", function (e) {
        ...
        if (modalsContainer) {
          modalsContainer.classList.remove("dimmed");
        }
        ...
        setTimeout(() => {
          ...
          modalOverlay.querySelector("#link_deploy_overlay").style.display =
            "none";
        }, 300);
      });
  } catch (error) {
    ...
    if (projectsMessage) {
      projectsMessage.textContent =
        "Não foi possível carregar os projetos. Verifique a conexão ou a API do GitHub.";
    }
  }
}
